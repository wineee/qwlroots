...
QWKeyboardGroup::QWKeyboardGroup(wlr_keyboard_group *handle, bool isOwner)
    : QObject(nullptr)
...
QWKeyboardGroup *QWKeyboardGroup::create()
{
    auto *handle = wlr_keyboard_group_create();
    if (!handle)
        return nullptr;
    return new QWKeyboardGroup(handle, true);
}
