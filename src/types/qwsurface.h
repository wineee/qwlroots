...
public:
    static QWSurface *create(QWDisplay *display, const char *name);

    inline wlr_seat *handle() const {
        return QWObject::handle<wlr_seat>();
    }

    static QWSurface *get(wlr_seat *handle);
    static QWSurface *from(wlr_seat *handle);
...
private:
    QWSurface(wlr_seat *handle, bool isOwner);
    ~QWSurface() = default;
};
